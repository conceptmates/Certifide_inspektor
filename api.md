...
5. **Reopen** (optional) — Flip a submitted (but not-yet-approved) inspection back to draft for further editing

**Editing is allowed any time until an admin approves the inspection.** Save-step works on both `draft` and `completed` records — only an approved inspection is locked. To move a `completed` inspection back into the step-by-step draft flow (and clear its `submitted_at`), call `/reopen`.
...
POST /{id}/reopen         → flip completed → draft (only if not yet approved)
  ↓
...
> Save-step also works directly on a `completed` (un-approved) inspection without
> reopening — `/reopen` is only needed if you want the record back in `draft`
> state (e.g. to clear `submitted_at` and resume the guided flow).
...
If called again with the same `registration_number` and vehicle, the same `inspection_id` is returned (idempotent — safe to retry on app restart).
...
Works while the inspection is **not yet approved** — i.e. `processing_status` is `"draft"` **or** `"completed"` (but `is_approved = false`). This lets the inspector keep editing after submitting, right up until an admin approves. Once approved, the record is locked and save-step returns 422.
...
| 422  | Inspection has been approved (locked — no longer editable) |
...
### 6. Reopen Inspection (edit after submitting)
...
- Owner-only (admins can reopen any).

> You don't strictly need this just to edit — `/save-step` already accepts edits
> on a `completed` (un-approved) record. Use `/reopen` when you specifically want
> the record back in `draft` state (clears `submitted_at`, removes it from
> review queues, restores the guided flow).
...
Admins see all inspections. Inspectors see only their own. Draft records are excluded.

#### Query Parameters

| Param              | Type    | Notes                                          |
| ------------------ | ------- | ---------------------------------------------- |
| `status`           | string  | `pending`, `approved`, `rejected` — admin only |
...
The `status` field in history responses maps these to: `"pending"`, `"approved"`, `"rejected"`.
